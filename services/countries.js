// The country list for the sign-in number field: ISO code, name, dial code.
//
// BUNDLED, NOT FETCHED — and that is the point.
//
// This used to come from /app_login/config, i.e. from Odoo's res.country. It
// meant the picker on the FIRST screen of the app could not be opened until a
// server had been reached, so every connection problem turned into a dead
// dropdown with nothing to explain it. Dial codes are static reference data;
// asking a server for them bought nothing and cost the one screen that has to
// work when everything else does not. WhatsApp's picker works offline too.
//
// Identified by ISO CODE, never by res.country id. Those ids are per-database
// and differ between installations, so a bundled list keyed on them would send
// the wrong country the moment the app pointed at a different Odoo. The server
// resolves the code to its own record.
//
// Shared shape with C:\Projects\SPA (data/countries.js) so the two apps agree on
...
export const COUNTRIES = [
  ...
  { code: "BV", name: "Bouvet Island", dial: "55" },
  ...
  { code: "DO", name: "Dominican Republic", dial: "1849" },
  ...
  { code: "XI", name: "Northern Ireland", dial: "44" },
  ...
  { code: "SX", name: "Sint Maarten (Dutch part)", dial: "1721" },
  ...
  { code: "UM", name: "USA Minor Outlying Islands", dial: "699" },
  ...
  { code: "VI", name: "Virgin Islands (USA)", dial: "1340" },
...
// Shown first — the places these users actually are, so the common cases need no
// typing. Everything else follows alphabetically.
export const POPULAR = ['IN', 'OM', 'AE', 'SA', 'QA', 'BH', 'KW', 'GB', 'US'];

export const COUNTRY_ROWS = [
  ...POPULAR.map((c) => COUNTRIES.find((x) => x.code === c)).filter(Boolean),
  ...COUNTRIES.filter((c) => !POPULAR.includes(c.code)),
];

// The default when nobody has chosen. Not read from the device locale, which
// would mean adding expo-localization to pre-select a dropdown.
export const DEFAULT_COUNTRY = 'IN';

// ISO alpha-2 -> flag emoji, by mapping each letter to its regional indicator
// symbol (A = U+1F1E6). No image assets and no 250 emoji literals: the flag is
// derived from the code the row already carries.
...
export function dialOf(code) {
  return countryByCode(code)?.dial || '91';
}

// Matches on name, dial code or ISO code. Digits typed without a leading "+"
// still match the dial code, so "968" finds Oman as readily as the name does.
export function matchCountry(c, query) {
