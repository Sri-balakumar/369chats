...
function Ticks({ status }) {
  ...
  return <Ionicons name="checkmark-done" size={14} color={read ? '#34B7F1' : COLORS.slate400} />;
}
    ...
    <View style={[s.row, mine ? s.rowMine : s.rowTheirs]}>
      ...
      >
        ...
        {msg.deleted ? (
          ...
          <View>
            <Image source={{ uri: msg.mediaUri }} style={s.media} resizeMode="cover" />
            {msg.kind === 'video' && (
          ...
          <View style={s.fileRow}>
            <View style={[s.fileChip, mine && s.fileChipMine]}>
              <Ionicons
                ...
                size={18} color={mine ? '#fff' : COLORS.primary}
              />
            </View>
            <View style={{ flex: 1 }}>
          ...
          <TouchableOpacity style={s.meet} activeOpacity={0.85} onPress={() => onMeet?.(msg.body)}>
            <View style={s.meetIcon}><Ionicons name="videocam" size={18} color="#fff" /></View>
            <View style={{ flex: 1 }}>
          ...
          <>
            ...
            <LinkedText style={[s.body, mine && s.bodyMine]}>{msg.body}</LinkedText>
          </>
        )}
...
const s = StyleSheet.create({
  ...
  mine: { backgroundColor: '#D9FDD3', borderBottomRightRadius: 4 },
  theirs: { backgroundColor: '#fff', borderBottomLeftRadius: 4 },

  author: { fontSize: 12.5, fontWeight: '800', color: COLORS.primary, marginBottom: 2 },

  reply: {
    borderLeftWidth: 3, borderRadius: RADIUS.sm, paddingHorizontal: SPACING.sm,
    paddingVertical: 4, marginBottom: SPACING.xs,
  },
  replyMine: { backgroundColor: 'rgba(0,0,0,0.06)', borderLeftColor: COLORS.green },
  replyTheirs: { backgroundColor: COLORS.slate50, borderLeftColor: COLORS.primary },
  replyAuthor: { fontSize: 11.5, fontWeight: '800', color: COLORS.primary },
  replyPreview: { fontSize: 12, color: COLORS.slate500 },

  body: { fontSize: 15, color: COLORS.ink, lineHeight: 20 },
  bodyMine: { color: '#0B1F14' },
  deleted: { fontStyle: 'italic', color: COLORS.slate500 },

  media: { width: 232, height: 232, borderRadius: RADIUS.lg, backgroundColor: COLORS.slate100 },
  ...
  fileChip: {
    width: 38, height: 38, borderRadius: 19, backgroundColor: '#EAF1FE',
    alignItems: 'center', justifyContent: 'center',
  },
  fileChipMine: { backgroundColor: COLORS.green },
  fileName: { fontSize: 14, fontWeight: '700', color: COLORS.ink },
  fileMeta: { fontSize: 11.5, color: COLORS.slate500, marginTop: 1 },

  link: { borderRadius: RADIUS.md, overflow: 'hidden', marginBottom: SPACING.xs, minWidth: 210 },
  linkMine: { backgroundColor: 'rgba(0,0,0,0.06)' },
  linkTheirs: { backgroundColor: COLORS.slate50 },
  linkImg: { width: '100%', height: 118, backgroundColor: COLORS.slate100 },
  linkBody: { paddingHorizontal: SPACING.md, paddingVertical: SPACING.sm, gap: 2 },
  linkTitle: { fontSize: 13.5, fontWeight: '800', color: COLORS.ink },
  linkDesc: { fontSize: 12, color: COLORS.slate500, lineHeight: 16 },
  linkUrl: { fontSize: 11, color: COLORS.link, marginTop: 1 },

  fwd: { flexDirection: 'row', alignItems: 'center', gap: 3, marginBottom: 2 },
  fwdTxt: { fontSize: 10.5, color: COLORS.slate400, fontStyle: 'italic', fontWeight: '600' },
  ...
  reactChipMine: { borderColor: COLORS.primary, backgroundColor: '#EAF1FE' },
  reactEmoji: { fontSize: 13 },
  reactCount: { fontSize: 11, fontWeight: '800', color: COLORS.slate500 },

  poll: { minWidth: 210, paddingVertical: 2 },
  pollQ: { fontSize: 15, fontWeight: '800', color: COLORS.ink, marginBottom: SPACING.sm },
  pollOpt: {
    borderRadius: RADIUS.md, borderWidth: 1, borderColor: COLORS.line,
    marginBottom: 6, overflow: 'hidden', backgroundColor: '#fff',
  },
  pollFill: { position: 'absolute', left: 0, top: 0, bottom: 0, backgroundColor: '#EEF2FB' },
  pollFillMine: { backgroundColor: '#DCE9FF' },
  pollOptRow: { flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 9, paddingVertical: 8 },
  pollTxt: { flex: 1, fontSize: 13.5, color: COLORS.ink, fontWeight: '600' },
  pollPct: { fontSize: 12, fontWeight: '800', color: COLORS.slate500 },
  pollTotal: { fontSize: 11.5, color: COLORS.slate400, marginTop: 2 },
  ...
  meetTitle: { fontSize: 14.5, fontWeight: '800', color: COLORS.ink },
  meetUrl: { fontSize: 11.5, color: COLORS.link, marginTop: 1 },

  meta: { flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', gap: 4, marginTop: 2 },
  metaTxt: { fontSize: 10.5, color: COLORS.slate400 },
  metaMine: { color: '#5A7A66' },

  systemWrap: { alignItems: 'center', marginVertical: SPACING.sm, paddingHorizontal: 30 },
  systemTxt: {
    fontSize: 11.5, color: COLORS.slate500, textAlign: 'center', fontWeight: '600',
